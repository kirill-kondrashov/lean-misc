# Plan: Erdős #142 Revised Research Program

Date: 2026-03-08

Revision: reset the active program from wrapper/regression expansion to mathematical debt
reduction.

## Objective

Replace the current self-extending packaging chain by a math-first research program with explicit
stop rules.

The active question is no longer:

```text
what additional theorem wrapper should be checked next?
```

It is:

```text
what is the next action that could actually change the mathematical debt profile of the repo?
```

## Status

Progress: `███████` `7 / 7`

## What This Revises

The completed packaging/regression chain did useful cleanup:

- it made the practical split route explicit;
- it made the negative `k = 3` route explicit;
- it made the off-path frontier route explicit at several intermediate layers.

But that chain has now crossed the point of diminishing mathematical returns.

## Stop Rules

From this point forward, a cycle is not active unless it does at least one of:

1. reduces the set of unproved mathematical debts;
2. adds a genuinely new source-backed theorem or audited no-go result;
3. falsifies or narrows the scope of an existing route;
4. isolates one concrete new mathematical target with measurable downstream consequence.

Corollary:

```text
do not open another cycle whose only effect is:
- a new `Nonempty ...` wrapper,
- another checker entry,
- another README synchronization,
- or another theorem-surface alias.
```

## Active Program

### Track A. Frontier debt ledger

Freeze the actual remaining debt list and rank it by mathematical leverage:

1. `k = 3` matched-profile strengthening beyond the audited `β = 1 / 12` split route;
2. `k = 4` profile-dominance / coupling debt;
3. `k ≥ 5` profile-dominance / coupling debt;
4. alternative integer-transfer or structured-object route not covered by the current barrier
   template.

Deliverable:

```text
one debt ledger with:
- source status,
- exact theorem target,
- reason it matters,
- reason current evidence does or does not support it.
```

### Track B. Source-first audit before more Lean packaging

Pick exactly one live debt from Track A and run a source-first audit.

Allowed outputs:

1. audited positive import target;
2. audited negative verdict;
3. precise statement that the repo's present theorem target is mis-specified.

Disallowed output:

```text
more theorem-surface packaging without new mathematical content.
```

### Track C. Negative-route scope control

Keep the negative `k = 3` route live only as a scoped theorem about the present architecture.

The program must continue to ask:

```text
what exact hypotheses does the barrier theorem really need,
and what would count as a real escape rather than a cosmetic reparameterization?
```

This prevents the negative route from becoming self-sealing.

### Track D. Lean work gated by math

Lean-side work continues only after Tracks A-C produce one of:

1. a new source-backed theorem target;
2. a revised no-go theorem;
3. a corrected route interface.

## Milestones

1. `[x]` Diagnose the current program drift.
   - verdict: wrapper/regression work has outrun mathematical debt reduction

2. `[x]` Freeze stop rules for future cycles.
   - no more packaging-only cycles as active research

3. `[x]` Add an explicit research-program critique.
   - note: `NOTES_problem142_research_program_critique_2026-03-08.md`

4. `[x]` Launch the first concrete post-reset cycle.
   - active cycle: `PLAN_erdos_problem_142_frontier_debt_ledger_2026-03-08.md`

5. `[x]` Build the frontier debt ledger.
   - first deliverable of the revised program
   - note: `NOTES_problem142_frontier_debt_ledger_2026-03-08.md`

6. `[x]` Run one source-first audit against the highest-leverage live debt.
   - chosen first target: `PLAN_erdos_problem_142_kge5_matched_profile_debt_audit_2026-03-08.md`

7. `[x]` Start exactly one math-bearing next cycle from the audited verdict.
   - active next cycle: `PLAN_erdos_problem_142_kge5_target_correction_2026-03-08.md`

## Current Active Cycle

- `PLAN_erdos_problem_142_kge5_target_correction_2026-03-08.md`

## Current Verdict

```text
The previous chain improved honesty and public structure, but it no longer deserves to be called
the active research program.

The revised active program is:
  freeze packaging,
  rank the real debts,
  audit one debt hard,
  and only then resume theorem construction.
```
