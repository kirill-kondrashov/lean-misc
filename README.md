...
- Quot.sound
- Classical.choice
✅ The proof of 'Erdos142.erdos_142_heterogeneous_source_backed_split_of_literature_heterogeneous_sourceBacked_route' is free of 'sorry' and uses only base axioms.
Axioms used:
- propext
- Quot.sound
- Classical.choice
...
- Erdos142.splitGap_k4_profile_dominance_frontier
- Erdos142.splitGap_kge5_profile_dominance_frontier
🟡 The proof of 'Erdos142.currentResearchStatus_exists_of_literature_heterogeneous_sourceBacked_route' is free of 'sorry' but relies on temporary allowed axiom debt.
Axioms used:
- propext
- Quot.sound
- Classical.choice
...
- The $k = 4$ branch now has two distinct split surfaces: the older local placeholder package `K4SourceBackedSplitWitness`, and the corrected broader-source heterogeneous package `K4HeterogeneousSourceBackedSplitWitness`; the canonical practical route now uses the latter; see [ErdosProblems/Problem142Literature.lean](./ErdosProblems/Problem142Literature.lean).
...
- These are exposed at the gap layer as `K3SourceBackedSplitGap`, the legacy `K4SourceBackedSplitGap`, the corrected `K4HeterogeneousSourceBackedSplitGap`, `K5SourceBackedSplitGap`, and the tail family `Kge6SourceBackedSplitGap`; the canonical practical target is now `MainAllHeterogeneousSourceBackedSplitGap`, while `MainAllSourceBackedSplitGap` remains as the older local placeholder route; see [ErdosProblems/Problem142Gap.lean](./ErdosProblems/Problem142Gap.lean).
- Therefore, on the active post-pivot route, the repository now has explicit source-backed split control for every branch, with the public practical route cut over to the corrected heterogeneous `k = 4` shape. The stronger unresolved content is no longer on the practical route; it survives only in the off-path matched-profile theorem family, including the stronger local target $U_4(N)=O(L_4(N))$.
...
- The current repository state is also packaged as one named object `Problem142CurrentResearchStatus`, whose fields expose the corrected practical route, the stabilized negative `k = 3` route, and the off-path frontier in one place; its theorem-level realization surface is `currentResearchStatus_exists_of_literature_heterogeneous_sourceBacked_route`; see [ErdosProblems/Problem142Gap.lean](./ErdosProblems/Problem142Gap.lean).
...
5. It first reorganizes the downstream gap so that $k = 3$ and then $k = 4$ are no longer part of the active unresolved matched-profile frontier, and then refines the route further so that $k = 5$ and finally every fixed $k \ge 6$ are also handled by explicit source-backed split packages; after the March 9, 2026 cutover, the canonical practical target is `MainAllHeterogeneousSourceBackedSplitGap`, while `MainAllSourceBackedSplitGap` is retained only as a legacy local route.
6. It packages the resulting canonical practical route into the named endpoint `Problem142AllHeterogeneousSourceBackedSplitData`, exports it to the statement-level theorem surfaces `HeterogeneousSourceBackedSplitRoute` and `erdos_142_heterogeneous_source_backed_split`, and provides the theorem `all_heterogeneous_source_backed_split_data_of_mainAllHeterogeneousSourceBackedSplitGap`, which gives all upper variants together with correct branchwise split data for $k = 3$, corrected heterogeneous $k = 4$, $k = 5$, and every fixed $k \ge 6$; see [ErdosProblems/Problem142Gap.lean](./ErdosProblems/Problem142Gap.lean).
